...
import type { AgentPayment, DeveloperAgent, PaymentStatus } from "@xone/types";
...
/**
 * Lists agents owned by a wallet (active + paused; excludes soft-deleted).
 * @param admin - Supabase admin
...
export async function updateDeveloperAgentLimits(
  ...
  if (!agent || (agent.status !== "active" && agent.status !== "paused")) {
...
/**
 * Pauses an agent wallet so API key payments are blocked until resume.
 * @param admin - Supabase admin
 * @param id - Agent id
 * @param ownerAddress - Owner wallet
 */
export async function pauseDeveloperAgent(
  ...
  return setDeveloperAgentStatus(admin, id, ownerAddress, "paused", "active");
}
...
export async function resumeDeveloperAgent(
  ...
  return setDeveloperAgentStatus(admin, id, ownerAddress, "active", "paused");
}

/**
 * Transitions agent status between active and paused.
 * @param admin - Supabase admin
 * @param id - Agent id
 * @param ownerAddress - Owner wallet
 ...
 */
async function setDeveloperAgentStatus(
  admin: SupabaseClient,
  id: string,
  ownerAddress: string,
  next: "active" | "paused",
  ...
  const agent = await getDeveloperAgentForOwner(admin, id, ownerAddress);
  if (!agent || agent.status !== expected) {
    throw new Error(
      expected === "active" ? "Agent is not active" : "Agent is not paused",
    );
  }
