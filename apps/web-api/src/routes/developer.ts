...
/**
 * POST /api/developer/agents/:id/pause — block machine payments until resumed.
 */
developer.post("/agents/:id/pause", async (c) => {
  const parsed = pauseResumeDeveloperAgentSchema.safeParse(
  ...
  try {
    const agent = await pauseDeveloperAgent(
      admin,
      c.req.param("id"),
      parsed.data.ownerAddress,
    );
...
/**
 * POST /api/developer/agents/:id/resume — re-enable a paused agent wallet.
 */
developer.post("/agents/:id/resume", async (c) => {
  const parsed = pauseResumeDeveloperAgentSchema.safeParse(
    await c.req.json().catch(() => ({})),
  );
  if (!parsed.success) {
  ...
  try {
    const agent = await resumeDeveloperAgent(
      admin,
      c.req.param("id"),
      parsed.data.ownerAddress,
    );
